...
exports.run = async (client, msg, args) => {
    ...
    let embed = new Discord.MessageEmbed().setAuthor(await Nekos.cat() + " \uD83D\uDCD6", process.env.AVATAR)
        .setColor(0xFFFFFF);
    for (const [cat, cmds] of Object.entries(categories)) {
        let str = [""], i = 0;
        cmds.forEach(command => {
            let cmd = commands.get(command);
            if (cmd.isNSFW === undefined || (cmd.isNSFW(command) && msg.channel.nsfw) || !cmd.isNSFW(command)) {
