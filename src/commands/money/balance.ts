...
const cmd = new Command('balance', `Get your current ${config.MONEY_NAME} amount.`, async i => {
    ...
    if (!target) {
        ...
        embed.setDescription(`${target.username}'s current balance is ${balance} ${config.MONEY_SYMBOL}.`);
    }
...
cmd.data.addUserOption(option => option.setName('member').setDescription('The member to check the balance of').setRequired(false));
export default cmd;